...
interface Character {
  ...
  damageDisplay: any[]; // Replace `any` with a more specific type if possible
  statusEffects: { name: string; duration: number; stack?: number }[];
  inventory: {
    equipment: any[]; // Replace `any` with a more specific type if possible
    consumables: any[]; // Replace `any` with a more specific type if possible
    miscItems: any[]; // Replace `any` with a more specific type if possible
    attunedItems: any[]; // Replace `any` with a more specific type if possible
  };
